...
| `--strict-mcp-config` | `ADAPTER_STRICT_MCP_CONFIG` | Use only the MCP servers this bridge passes explicitly, ignoring the host's user-scope servers, project `.mcp.json`, plugins and agent frontmatter (default `true`; pass `false` to inherit them as upstream does). Local patch, see `.local_patch_version`. |
